...
describe "Admin manages initiatives", type: :system do
  ...
  let(:status1) { create :status, organization: organization }
  let(:status2) { create :status, organization: organization }
  ...
  def initiative_with_status(status)
    Decidim::Initiative.find_by(decidim_status_id: status)
  end

  def initiative_without_status(status)
    Decidim::Initiative.where.not(decidim_status_id: status).sample
  end
  ...
  describe "listing initiatives" do
    ...
    Decidim::Status.all.each do |status|
      i18n_status = status.name[I18n.locale.to_s]

      context "filtering collection by status: #{i18n_status}" do
        before do
          create(:initiative, organization: organization, status: status1)
          create(:initiative, organization: organization, area: status2)
        end

        it_behaves_like "a filtered collection", options: "Status", filter: i18n_status do
          let(:in_filter) { translated(initiative_with_status(status).title) }
          let(:not_in_filter) { translated(initiative_without_status(status).title) }
        end
      end
    end
